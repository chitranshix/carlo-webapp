    ...
    toolbar.innerHTML = `
        <div class="flex flex-wrap items-center gap-3 text-sm">
            ...
            <div class="relative" id="pos-dropdown-container">
                <button id="pos-menu-btn" ${filtersDisabledAttrs} class="px-3.5 py-2 rounded-lg border text-xs flex items-center space-x-2 transition shadow-xs select-none ${posBtnClasses}">
                    ${posButtonIcon}
            ...
            <div class="relative" id="status-dropdown-container">
                <button id="status-menu-btn" ${filtersDisabledAttrs} class="px-3.5 py-2 rounded-lg border text-xs flex items-center space-x-2 transition shadow-xs select-none ${statusBtnClasses}">
                    ${statusButtonIcon}
                ...
                <div id="status-menu-list" class="hidden absolute left-0 mt-1 w-48 bg-white border border-[#E1E1E1] rounded-lg shadow-xl z-20 py-1 text-xs">
                    <div class="px-3 py-2 hover:bg-gray-100 cursor-pointer font-medium flex items-center justify-between" data-value="all">
                        <span class="flex items-center space-x-2">
                            ${allIcon('w-3.5 h-3.5 text-gray-500')}
                    ...
                    <div class="px-3 py-2 hover:bg-gray-100 cursor-pointer flex items-center justify-between" data-value="Unseen">
                        <span class="flex items-center space-x-2">
                            ${statusIcon('Unseen', 'w-3.5 h-3.5 text-gray-400')}
                            <span>Unseen</span>
                        </span>
        ...
        <div class="flex items-center space-x-3 text-xs text-[#767676]">
            <button id="test-mode-btn" title="${testProgress ? 'Click to view all words' : 'Click to start a test'}" class="flex items-center space-x-1.5 px-3.5 py-2 rounded-lg transition select-none cursor-pointer font-medium shadow-xs ${testProgress
                ? 'bg-white border border-[#E1E1E1] text-[#333333] hover:bg-gray-50'
